...
def _safe_path_component(name: str) -> str:
    ...
    sanitized = "".join(
        ch if ch.isalnum() or ch in {"_", "-", "."} else "_" for ch in sanitized
    )
...
def _resolve_image_format(raw: Optional[object]) -> str:
    if raw is None:
        return "jpg"
    value = str(raw).strip().lower()
    if value not in {"jpg", "png"}:
        raise ConfigError("image_format 仅支持 'jpg' 或 'png'")
    return value
    ...
    *,
    ...
    if config.filename_time_source == "header":
        ...
        if topic_name not in warning_cache:
            logger.warning(
                "topic '%s' 的 header.stamp 为空，将使用 bag 时间戳", topic_name
            )
...
def _extract_scalar_column(data: np.ndarray, field_name: str) -> np.ndarray:
    ...
    if array.ndim == 2:
        if array.shape[1] != 1:
            raise RuntimeError(
                f"字段 '{field_name}' 的 count={array.shape[1]} 暂不支持导出"
            )
...
def build_effective_config(
    ...
    *,
    ...
    lidar_topics_yaml = _normalize_topic_list(
        (
            lidar_section.get("topics")
            if lidar_section
            else yaml_config.get("lidar_topics")
        )
    )
    ...
    camera_section = (
        camera_section_raw if isinstance(camera_section_raw, dict) else None
    )
    camera_topics_yaml = _normalize_topic_list(
        (
            camera_section.get("topics")
            if camera_section
            else yaml_config.get("camera_topics")
        )
    )
    ...
    include_tf_static_yaml = _resolve_bool(
        (
            tf_section.get("enabled")
            if tf_section
            else yaml_config.get("tf_static_enabled")
        ),
        True,
    )
    ...
    lidar_subdir = _resolve_subdir(
        cli_lidar_subdir
        or (
            yaml_config.get("lidar_subdir")
            if yaml_config.get("lidar_subdir")
            else (lidar_section.get("output_subdir") if lidar_section else None)
        ),
        "lidar",
    )
    camera_subdir = _resolve_subdir(
        cli_camera_subdir
        or (
            yaml_config.get("camera_subdir")
            if yaml_config.get("camera_subdir")
            else (camera_section.get("output_subdir") if camera_section else None)
        ),
        "camera",
    )
    calib_subdir = _resolve_subdir(
        cli_calib_subdir
        or (
            yaml_config.get("calib_subdir")
            if yaml_config.get("calib_subdir")
            else (tf_section.get("output_subdir") if tf_section else None)
        ),
        "calib",
    )

    filename_style = _resolve_filename_style(
        cli_filename_style or yaml_config.get("filename_style")
    )
    filename_time_source = _resolve_time_source(
        cli_time_source or yaml_config.get("filename_time_source")
    )
    ...
    if filename_style == "datetime":
        ...
        filename_time_format = (
            _resolve_time_format(time_format_raw)
            if time_format_raw
            else "%Y%m%dT%H%M%S_%f"
        )
    ...
    return ExportConfig(
        ...
        pointcloud_format=_resolve_pointcloud_format(
            cli_pointcloud_format
            or (
                lidar_section.get("output_format")
                if lidar_section
                else yaml_config.get("pointcloud_format")
            )
        ),
        pcd_format=_resolve_pcd_format(
            cli_pcd_format
            or (
                lidar_section.get("output_pcd_format")
                if lidar_section
                else yaml_config.get("pcd_format")
            )
        ),
        image_format=_resolve_image_format(
            cli_image_format
            or (
                camera_section.get("output_format")
                if camera_section
                else yaml_config.get("image_format")
            )
        ),
    )
...
def _write_pointcloud_binary_compressed(
    ...
    *,
    ...
    if pcd_fmt == "compressed":
        ...
        body = (
            struct.pack("<II", len(compressed_data), uncompressed_size)
            + compressed_data
        )
        output_path.write_bytes(headers_blob + body)
        return
...
def _compose_static_transforms(
    registry: Dict[str, _StaticTransform], base_frame: str
) -> Dict[str, np.ndarray]:
...
def _save_extrinsics(
    ...
    for frame_id, original in frames_to_export.items():
        ...
        if frame_id not in composed:
            logger.warning(
                "frame '%s' 无法通过 tf_static 转换到 '%s'，跳过外参导出",
                frame_id,
                config.base_frame,
            )
            continue
...
def export_bag_assets(config: ExportConfig) -> None:
    ...
    if config.output_root.exists():
        if config.overwrite:
            ...
            raise ConfigError(
                f"输出目录 {config.output_root} 已存在，如需覆盖请使用 --overwrite"
            )
    ...
    topics_by_name = {
        topic.topic_metadata.name: topic.topic_metadata for topic in topics_info
    }

    available_topics = set(topics_by_name.keys())

    def _validate_topics(
        requested: Sequence[str], allowed_types: Set[str], kind: str
    ) -> Dict[str, str]:
        ...
        for name in requested:
            ...
            if topic_type not in allowed_types:
                raise ConfigError(
                    f"topic '{name}' 类型为 {topic_type}，不是预期的 {kind} 类型"
                )
            selection[name] = topic_type
        return selection

    if config.lidar_topics:
        lidar_topics = _validate_topics(
            config.lidar_topics, KNOWN_LIDAR_TYPES, "激光雷达"
        )
    else:
        lidar_topics = {
            meta.name: meta.type
    ...
    if config.camera_topics:
        camera_topics = _validate_topics(
            config.camera_topics, KNOWN_CAMERA_TYPES, "相机"
        )
    else:
        camera_topics = {
            meta.name: meta.type
    ...
    tf_static_present = (
        TF_STATIC_TOPIC in topics_by_name
        and topics_by_name[TF_STATIC_TOPIC].type == TF_STATIC_TYPE
    )
    if config.include_tf_static and not tf_static_present:
        logger.warning("bag 中没有 /tf_static，无法导出外参")

    message_counts = {
        topic.topic_metadata.name: topic.message_count for topic in topics_info
    }
    ...
    storage_options = rosbag2_py.StorageOptions(
        uri=str(config.input_bag), storage_id=storage_id
    )
    ...
    with progress:
        lidar_task = (
            progress.add_task("点云", total=lidar_total or None)
            if getattr(progress, "add_task", None)
            else None
        )
        camera_task = (
            progress.add_task("图像", total=camera_total or None)
            if getattr(progress, "add_task", None)
            else None
        )
        ...
        while reader.has_next():
            ...
            if topic_name in lidar_topics:
                ...
                if not isinstance(message, PointCloud2):
                    raise RuntimeError(
                        f"topic '{topic_name}' 的消息类型不是 PointCloud2"
                    )
                ...
                if not frame_id:
                    logger.warning(
                        "topic '%s' 的 PointCloud2 消息缺少 frame_id，已跳过",
                        topic_name,
                    )
                    continue
                ...
                if not frame_id:
                    logger.warning(
                        "topic '%s' 的图像消息缺少 frame_id，已跳过", topic_name
                    )
                    continue
                ...
                file_dir = (
                    config.output_root
                    / config.camera_subdir
                    / _safe_path_component(frame_id)
                )

                target_ext = f".{config.image_format}"
                file_name = _build_unique_filename(
                    directory=file_dir,
                    base_name=token,
                ...
                if topic_type == "sensor_msgs/msg/Image":
                    ...
                    save_fmt = "JPEG" if config.image_format == "jpg" else "PNG"
                    image.save(output_path, format=save_fmt)
                    camera_count += 1

                elif topic_type == "sensor_msgs/msg/CompressedImage":
                    if not isinstance(message, CompressedImage):
                        raise RuntimeError(
                            f"topic '{topic_name}' 的消息类型不是 CompressedImage"
                        )

                    # 尝试直写优化
                    fmt_lower = message.format.lower()
                    is_jpeg_source = "jpeg" in fmt_lower or "jpg" in fmt_lower
                    is_png_source = "png" in fmt_lower

                    # 如果源格式与目标格式一致，直接写入原始数据
                    if (config.image_format == "jpg" and is_jpeg_source) or (
                        config.image_format == "png" and is_png_source
                    ):
                        if not message.data:
                            logger.warning("topic '%s' 的压缩图像数据为空", topic_name)
                        else:
                ...
                for transform in message.transforms:
                    ...
                    if not child:
                        continue
                    static_transforms[child] = _StaticTransform(
                        parent=parent,
                        child=child,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="将 rosbag2 中的点云、图像与外参导出为通用格式。"
    )
    ...
    parser.add_argument(
        "--base-frame", type=str, help="外参统一到的基准坐标系，默认 base_link"
    )
    parser.add_argument(
        "--lidar-topic",
        action="append",
        default=[],
        help="指定需要导出的激光雷达 topic，可重复",
    )
    parser.add_argument(
        "--camera-topic",
        action="append",
        default=[],
        help="指定需要导出的相机 topic，可重复",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["ns", "datetime"],
        help="文件名时间格式，ns 为纳秒整数，datetime 使用 strftime 格式",
    )
    parser.add_argument(
        "--filename-time-source",
        type=str,
        choices=["header", "bag"],
        help="文件名时间来源，header 或 bag",
    )
    parser.add_argument(
        "--filename-time-format",
        type=str,
        help="当文件名样式为 datetime 时使用的 strftime 格式，默认 %Y%m%dT%H%M%S_%f",
    )
    parser.add_argument(
        "--pointcloud-format",
        type=str,
        choices=["auto", "xyz", "xyzi", "xyzit"],
        help="点云导出格式，auto/xyz/xyzi/xyzit，默认为 auto",
    )
    parser.add_argument(
        "--pcd-format",
        type=str,
        choices=["uncompressed", "ascii", "compressed"],
        help="生成的 PCD 存储格式：uncompressed/ascii/compressed，默认 uncompressed",
    )
    parser.add_argument(
        "--image-format",
        type=str,
        choices=["jpg", "png"],
        help="图像导出格式：jpg/png，默认为 jpg",
    )
    parser.add_argument(
        "--disable-tf-static", action="store_true", help="禁用 tf_static 外参导出"
    )
    parser.add_argument(
        "--overwrite", action="store_true", help="如果输出目录已存在则删除重建"
    )
    return parser
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        config = build_effective_config(
            ...
            cli_image_format=args.image_format,
        )
